    ...
    for year in year_avail:
        ...
        for i in np.arange(0, nrows):
            for j in np.arange(0,ncols):
                ...
                #map_array[i, j, 0] = int(category) old version
                # re-order the classes
                if category == 5:
                    map_array[i, j, 0] = 1
                elif category == 4:
                    map_array[i, j, 0] = 2
                    ...
                    map_array[i, j, 0] = 8
                elif (category>=7 or category<=15):
                    map_array[i, j, 0] = category + 2
                else:
...
def main(tile_name):
    ...
    output_folder_path = r'/projectnb/landsat/projects/ABOVE/CCDC/{0}/new_map/out_classes'.format(tile_name)
