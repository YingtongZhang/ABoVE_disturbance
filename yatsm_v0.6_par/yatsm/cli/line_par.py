...
def line(ctx, config, job_number, total_jobs,
    ...
    for line in job_lines:
        ...
        #output = [do_yatsm_loop(df,Y,dates,col,line,cfg) for col in np.arange(Y.shape[-1])]

        # Y.take(col, axis=2)
        # Y[:, :, col]
        output = Parallel(n_jobs=-1, max_nbytes=None)(delayed(do_yatsm_loop)
            (df,Y.take(col, axis=2),dates,col,line,cfg) 
...
def do_yatsm_loop(df,_Y,dates,col,line,cfg):
