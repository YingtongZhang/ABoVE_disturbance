...
class _GDALStackReader(object):
    ...
    def _read_row(self, row):
        data = np.empty((self.n_band, self.n_image, self.n_col),np.int16)
#                        self.datatype)
        for i, ds_bands in enumerate(self.dataset_bands):
            for n_b, band in enumerate(ds_bands):
#                data[n_b, i, :] = band.ReadAsArray(xoff = 0, yoff = row, win_xsize = self.n_col, win_ysize = 1).astype(np.int16)
                data[n_b, i, :] = band.ReadAsArray(xoff = 0, yoff = int(row), win_xsize = self.n_col, win_ysize = 1, buf_xsize = self.n_col, buf_ysize=1)
        return data
