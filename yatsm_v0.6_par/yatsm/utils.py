...
def write_output(raster, output, grid_info, gdal_frmt, band_names=None, ndv=-9999):
    ...
    ## the geo transform goes - ulx, pix_x, easting, uly, northing, pix_y
...
def iter_records(records, warn_on_empty=True, yield_filename=False):
...
def get_record_x(records, x, warn_on_empty=True, yield_filename=False):
    """ Iterates over records, returning result NumPy array
...
def convert_to_ord(date_str):
    return(dt.toordinal(dt.strptime(date_str,"%Y%j")))
