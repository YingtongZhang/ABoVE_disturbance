...
def map_nbr(npz_file_path, img_file_path, outdir_path):
    """
    Read in .npz.npy file and make maps
    Args:
        npz_file_path: input npz_file_path of npr metrics of all breaks
        outdir_path: output directory path of maps of dnpr metrics for each year
    Return:
        None
    """
    data_arr = np.load(npz_file_path, allow_pickle=True)
    print('Loading .npz.npy file...')    
...
def main(tile_name):
    ...
    img_file_path = r'/projectnb/modislc/users/jonwang/data/rf/rast/tc_20180416_noGeo_k55_pam_rf/{0}/remap/{0}_1985_tc_20180416_noGeo_k55_pam_rf_remap.tif'.format(tile_name)
    outdir_path = r'/projectnb/landsat/projects/ABOVE/CCDC/{0}/new_map/out_tc_pre'.format(tile_name)
    # outdir_path = r'/projectnb/landsat/users/zhangyt/above/CCDC/{0}/out_nbr'.format(tile_name)  # ******uncomment me****** #
