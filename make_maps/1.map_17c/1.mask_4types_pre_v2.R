...
output_dir = "/projectnb/landsat/projects/ABOVE/CCDC/"%+%tile_name%+%"/new_map/out_tc_4type/"
...
ras_list <- list.files(tc_dir,pattern="*.tif$", full.names=T) # 29 files in total, start from 1985
LC_list <- list.files(LC_map_dir,pattern="*remap.tif$", full.names=T) # 29 files in total, start from 1984

for (i in 1:29){
  layers = stack(ras_list[i])
  year = 1984 + i  
  LC_pre <- stack(LC_list[i])
  LC_aft <- stack(LC_list[i+1])
  LC_FF <- LC_pre
  LC_FN <- LC_pre
  LC_NF <- LC_pre
  LC_NN <- LC_pre
