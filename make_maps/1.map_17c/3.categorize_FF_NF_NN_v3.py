...
def classify_FF(tile_name, FF_tc_folder_path, FF_output_folder_path):
    ...
    for year in year_avail:
        ...
        # write FF_class, FF-decline-1, FF-growth-2
        for i in np.arange(0, nrows):
            for j in np.arange(0,ncols):
                ...
                if ((dw > 20) and (dg > 20)): 
                    map_array[i,j,0] = 2 
                elif ((dw < -20 and dw > -30000) and (dg < -20 and dg > -30000)):
                    map_array[i,j,0] = 1
...
def classify_NN(tile_name, NN_tc_folder_path, NN_output_folder_path):
    ...
    for year in year_avail:
        ...
        """
        # write NN_class, (class 9 to 17)
        #Class(dw>10 and dg>10)=9             Class(-30000<dw<-10 and dg>10)=10           Class(-10<dw<10 and dg>10)=11
        #Class(dw>10 and -30000<dg<-10)=12    Class(-30000<dw<-10 and -30000<dg<-10)=13  Class(-10<dw<10 and -30000<dg<-10)=14
        #Class(dw>10 and -10<dg<10)=15        Class(-30000<dw<-10 and -10<dg<10)=16      Class(-10<dw<10 and -10<dg<10)=17
        """
        for i in np.arange(0, nrows):
            for j in np.arange(0,ncols):
                dg = NN_array[1, i, j]
...
def main(tile_name):
    ...
    tc_folder_path = r'/projectnb/landsat/projects/ABOVE/CCDC/{0}/new_map/out_tc_4type'.format(tile_name)
    output_folder_path = r'/projectnb/landsat/projects/ABOVE/CCDC/{0}/new_map/out_category'.format(tile_name)
